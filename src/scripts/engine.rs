...
impl ScriptEngine {
    ...
    fn discover_scripts(config: &ScriptConfig) -> Result<Vec<ScriptFile>> {
        ...
        for dir in &config.directories {
            if !dir.exists() {
                // Try to create user script directories
                if let Some(home) = dirs::home_dir() {
                    let user_scripts = home.join(".phobos/scripts");
                    if dir == &user_scripts {
                        if let Err(e) = std::fs::create_dir_all(dir) {
                            debug!("Failed to create user script directory {:?}: {}", dir, e);
                        } else {
                            info!("Created user script directory: {:?}", dir);
                            // Create example script
                            Self::create_example_script(dir)?;
                        }
                    }
                }
                
                // Check if directory exists now, otherwise skip
                if !dir.exists() {
                    debug!("Script directory does not exist: {:?}", dir);
                    continue;
                }
            }
